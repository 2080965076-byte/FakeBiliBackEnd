...
public class DanmuWebSocketServer {
    ...
    private static JwtUtil jwtUtil;              // JWT工具类，用于token验证
    private static RedisUtil redisUtil;           // Redis工具类，用于缓存操作
    private static DanmuMapper danmuMapper;       // 弹幕数据访问层
    private static VideoStatsService videoStatsService; // 视频统计服务

    /**
     * 依赖注入方法
     * @param jwtUtil JWT工具类实例
     * @param redisUtil Redis工具类实例
     * @param danmuMapper 弹幕数据访问层实例
     * @param videoStatsService 视频统计服务实例
     */
    @Autowired
    ...
    private static final Map<String, Set<Session>> videoConnectionMap = new ConcurrentHashMap<>(); // 存储视频ID和对应WebSocket连接的映射关系
...
//        System.out.println("关闭连接，当前观看人数: " + videoConnectionMap.get(vid).size());
    ...
    public void sendMessage(String vid, String text) {
        ...
        set.parallelStream().forEach(session -> {
            try {
                session.getBasicRemote().sendText(text);//websocket发送消息
            } catch (Exception e) {
                e.printStackTrace();
            }
        });
